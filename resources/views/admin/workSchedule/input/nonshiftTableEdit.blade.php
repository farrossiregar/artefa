                  ...
                  ?>
                    <tr>
                      ...
                      <td width="10%" @if(in_array($getDay['tgl'], $arrayTglMerah)) bgcolor="#eda8a8" @endif>
                        <select style="width: 42px" class="optHour not-active" @if(in_array($getDay['tgl'], $arrayTglMerah)) style="background: #eda8a8;" @endif id="{{$employee->id}}&{{$getDay['tgl']}}" name="{{$employee->id}}&{{$getDay['tgl']}}">
                          @if(!in_array($getDay['days'], ['Sun']))
                            @foreach($schedules->where('day',$getDay['days']) as $schedule)
                              ...
                              @if((in_array($getDay['days'], ['Mon','Tue','Wed','Thu','Fri'])) && (!in_array($getDay['tgl'], $arrayTglMerah)))
                                <option value="{{$employee->nik}}&{{$getDay['tgl']}}&{{$schedule->id}}" @foreach($nonshift_schedules as $data) @if(($data->nik == $employee->nik) && ($data->date == $getDay['tgl']) && ($data->schedule_code == $schedule->id)) selected="true" @endif @endforeach>
                              ...
                              @elseif($getDay['days']=='Sat')
                                ...
                                <option value="{{$employee->nik}}&{{$getDay['tgl']}}&0" @foreach($nonshift_schedules as $data) @if(($data->nik == $employee->nik) && ($data->date == $getDay['tgl']) && ($data->schedule_code == $schedule->id) && (!empty($data->remark))) selected="true" @endif @endforeach><span style="display: table;margin: auto;">Tukar Jadwal</span></option>
                                <option value="{{$employee->nik}}&{{$getDay['tgl']}}&{{$schedule->id}}" @foreach($nonshift_schedules as $data) @if(($data->nik == $employee->nik) && ($data->date == $getDay['tgl']) && ($data->schedule_code == $schedule->id) && (empty($data->remark))) selected="true" @endif @endforeach>
                                  <span style="display: table;margin: auto;">{{$schedule->day}} - ({{$start}} - {{$end}})</span>
                                </option>
                              @else
...
<div class="col-md-12" style="padding-top: 10px">
  <div class="row">
    <button class="btn btn-warning" id="btneditScheduleShift" onclick="editScheduleShift({{$employees}})"><i class="icon-pencil"></i> Edit</button>
    <button class="btn btn-success hide" id="btnsaveScheduleShift" onclick="saveScheduleNonShift({{$employees}})"><i class="icon-ok"></i> Simpan</button>
