    ...
    <div class="zui-wrapper">
        <div class="zui-scroller">
            <table class="zui-table" border="1">
                <thead>
                    <tr>
                        <th class="zui-sticky-col"  valign="center" bgcolor="#6cddbf" style="vertical-align: middle;"><div align="center" style="width: 150px;">Nama</div></th>
                    ...
                    ?>
                      <tr>
                        ...
                        <td width="10%" @if(in_array($value, $arrayTglMerah)) bgcolor="#eda8a8" @endif>
                            <select style="width: 42px;" class="optHour not-active" @if(in_array($value, $arrayTglMerah)) style="background: #eda8a8;" @endif id="{{$employee->id}}&{{$i}}" name="{{$employee->id}}&{{$i}}">
                              ...
                              @endphp
                              <option  value="{{$employee->nik}}&{{$i}}-{{$schedule->code}}" @foreach($shiftSchedules as $shiftSchedule) 
                                @if(($shiftSchedule->schedule_code == $schedule->code) && ($employee->nik == $shiftSchedule->nik) && ($value == $shiftSchedule->date)) selected="true" disable="true" @endif @endforeach>
  ...
  <div class="col-md-12" style="padding-top: 10px">
    <div class="row">
      ...
      <button class="btn btn-success hide" id="btnsaveScheduleShift" onclick="saveScheduleShift({{$employees}})"><i class="icon-ok"></i> Simpan</button>
